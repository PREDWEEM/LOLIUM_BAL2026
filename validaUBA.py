# -*- coding: utf-8 -*-
# ===============================================================
# 🌾 PREDWEEM INTEGRAL vK4.9.8 — LOLIUM BALCARCE 2026 (VALIDACIÓN DE CAMPO)
# Actualización:
...
# - Filtro de Indulto para Falsos Positivos y detección de cohortes.
# ===============================================================
...
def load_data(file_uploader, default_name):
    """
    Carga robusta para archivos crudos con encabezados múltiples (ej. Lucas Royo).
    """
    if file_uploader:
        ...
        # Limpieza dinámica: Buscar la fila que contiene 'Fecha' o 'Fecha / Hora'
        if 'Fecha' not in df.columns and 'FECHA' not in df.columns and 'Fecha / Hora' not in df.columns:
...
archivo_meteo = st.sidebar.file_uploader("1. Clima (Balcarce/Royo)", type=["xlsx", "csv"])
archivo_campo = st.sidebar.file_uploader("2. Campo (Recuentos Royo)", type=["xlsx", "csv"])
...
if df_meteo_raw is not None and modelo_ann is not None:
    ...
    # --- PREPROCESAMIENTO CAMPO (Adaptado para Lucas Royo) ---
    df_campo = None
    col_fecha = None
    col_plm2 = None
    ...
    if df_campo_raw is not None:
        ...
        if len(cols_reps) > 0:
            ...
            for col in cols_reps:
                ...
                df_campo[col] = pd.to_numeric(df_campo[col], errors='coerce').fillna(0)
            # Calcular el promedio de las repeticiones
    ...
    # Balance Hídrico con Secado Exponencial
