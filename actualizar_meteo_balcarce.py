...
def descargar_siga(fecha_inicio: date, fecha_fin: date) -> tuple[bytes, str, str]:
    ...
    nombre = coincidencia.group(1) if coincidencia else Path(url).name or "siga_balcarce_descarga.xls"
...
def cargar_pronostico_ecmwf() -> pd.DataFrame:
    ...
    archivo = DIRECTORIO_PRONOSTICOS / f"ecmwf_ifs_ens_025_balcarce_{marca}.csv"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Actualiza meteo_daily.csv para PREDWEEM Balcarce.")
