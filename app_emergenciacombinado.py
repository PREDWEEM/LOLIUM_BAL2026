...
# - ADAPTACIÓN BALCARCE: Coordenadas fijas en LAT=-37.7664 y LON=-58.2999 para ET0 Hargreaves.
# - IDENTIDAD: PREDWEEM by GUILLERMO R. CHANTRE.
# - LATENCIA INICIAL: Bloqueo estricto de emergencia los primeros 25 días del año.
# - ESPECÍFICO BALCARCE: Preservación absoluta del Modulador de Agotamiento Demográfico por pulsos.
# - VALIDACIÓN POR EVENTO REAL: Incorporación de la Integración Dinámica por Intervalo Variable
#   (Event-to-Event) para mitigar distorsiones de varianza en frecuencias de 7 a 21 días.
# - OPTIMIZADOR 2D: Barrido enfocado puramente en la bio-física del suelo (W_Max y Ke) usando ventanas reales.
...
if 'arranque_fase' not in st.session_state:
    ...
    st.info("🚜 **Iniciando Servidor PREDWEEM Balcarce...** Cargando motores dinámicos por evento.")
    st.progress(20)
...
# ---------------------------------------------------------
# 3. LÓGICA TÉCNICA Y RESTRUCTURACIÓN POR EVENTO REAL
# ---------------------------------------------------------
...
# ---------------------------------------------------------
# 3.5 MÓDULO OPTIMIZADOR 2D (CALIBRACIÓN BIO-FÍSICA BALCARCE)
# ---------------------------------------------------------
def optimizar_parametros_hidricos_2d(df_meteo, df_campo, modelo_ann, latitud_balcarce=-37.7664):
    df = df_meteo.copy()
    ...
    for w_max in rango_w_max:
        for ke in rango_ke:
            ...
            # Agotamiento demográfico de Balcarce dentro de optimización
            df_sim = aplicar_patron_agotamiento(df_sim)
...
dga_optimo = st.sidebar.number_input("Objetivo Control", value=800, step=50)
dga_critico = st.sidebar.number_input("Límite Ventana", value=900, step=50)
...
st.sidebar.info("🔬 **Modo Event-to-Event Activado**: Las ventanas de validación se auto-ajustan dinámicamente según el calendario real de tus datos de campo (7-21 días), protegiendo la varianza de flujos.")
    ...
    if st.button("Ejecutar Barrido Hídrico"):
        if df_meteo_raw is not None and df_campo_raw is not None and modelo_ann is not None:
            with st.spinner('Ejecutando iteraciones hídricas puras por evento...'):
                ...
                tabla_optima = optimizar_parametros_hidricos_2d(df_meteo_opt, df_campo_opt, modelo_ann, latitud_balcarce=-37.7664)
...
if df_meteo_raw is not None and modelo_ann is not None:
    ...
    # BALCARCE: Modulador de Agotamiento Demográfico y Techo Estricto 0-1
    ...
    st.plotly_chart(go.Figure(data=go.Heatmap(z=[df["EMERREL"].values], x=df["Fecha"], y=["Emergencia"], colorscale=colorscale_hard, zmin=0, zmax=1, showscale=False)).update_layout(height=120, margin=dict(t=30, b=0, l=10, r=10), title="Mapa de Riesgo Temporal (Tasa Diaria Balcarce)"), use_container_width=True)
    ...
    with tab2:
        st.header("💧 Dinámica Hídrica del Suelo (Balcarce)")
    ...
    st.sidebar.download_button("📥 Descargar Reporte Balcarce", output.getvalue(), "PREDWEEM_Integral_Balcarce_vK4_9_15.xlsx")

else:
    st.info("👋 Bienvenido a PREDWEEM. Cargue los datos climáticos de Balcarce para comenzar.")
