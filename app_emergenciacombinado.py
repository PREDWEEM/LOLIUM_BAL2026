...
# - ADAPTACIÓN BALCARCE: Coordenadas mantenidas estrictamente en -38.45 según indicación.
...
with st.expander("📂 1. Datos del Lote", expanded=True):
    ...
    with col_upload:
        archivo_usuario = st.file_uploader("Subir Clima Manual (BALCARCE)", type=["xlsx", "csv"])
...
umbral_er = st.sidebar.slider("Umbral Tasa Diaria (Detección pico)", 0.05, 0.80, 0.0)
...
if df is not None and modelo_ann is not None:
    ...
    st.sidebar.download_button("📥 Descargar Reporte", output.getvalue(), "PREDWEEM_Operativo_Balcarce_vK4_9_8.xlsx")
