...
if df is not None and modelo_ann is not None:
    ...
    # Lluvia acumulada en una ventana móvil de 21 días.
    df["Prec_sum_21d"] = df["Prec"].rolling(window=21, min_periods=1).sum()

    # Penalización suave: sigmoide centrada en 15 mm.
    df["Hydric_Factor"] = 1 / (1 + np.exp(-0.4 * (df["Prec_sum_21d"] - 15)))
    df["EMERREL"] = df["EMERREL"] * df["Hydric_Factor"]

    # Relajación dinámica:
    # - si la lluvia acumulada supera 50 mm, no se aplica bloqueo;
    # - si no, se bloquea la emergencia hasta el día juliano 25.
    jd_thresholds = np.where(df["Prec_sum_21d"] > 50, 0, 25)
    mask_bloqueo = df["Julian_days"].to_numpy() <= jd_thresholds
    df.loc[mask_bloqueo, "EMERREL"] = 0.0
    ...
    colorscale_hard = [
        [0.0, "green"], [0.14, "green"],
        [0.15, "yellow"], [0.34, "yellow"],
        [0.35, "red"], [1.0, "red"]
    ]
    ...
    with tab4:
        ...
        fig_bio.add_trace(go.Scatter(
            x=x_temps, y=y_tt, mode="lines",
            line=dict(color="#2563eb", width=4), fill="tozeroy"
        ))
